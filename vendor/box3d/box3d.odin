...
AllocFcn :: rawptr  /* ? void *(int32_t, int32_t) */

FreeFcn :: rawptr  /* ? void (void *) */

AssertFcn :: rawptr  /* ? int (const char *, const char *, int) */

LogFcn :: rawptr  /* ? void (const char *) */
...
RecQueryType :: enum c.int {
	recQueryOverlapAABB = 0,
	recQueryOverlapShape = 1,
	recQueryCastRay = 2,
	recQueryCastShape = 3,
	recQueryCastRayClosest = 4,
	recQueryCastMover = 5,
	recQueryCollideMover = 6,
}
...
foreign lib {
	...
	GetTicks :: proc() -> c.ulong ---
	// Get the milliseconds passed from an initial tick value.
	GetMilliseconds :: proc(ticks: c.ulong) -> f32 ---
	// Get the milliseconds passed from an initial tick value.
	GetMillisecondsAndReset :: proc(ticks: ^c.ulong) -> f32 ---
	// Yield to be used in a busy loop.
	Yield :: proc() ---
	// Sleep the current thread for a number of milliseconds.
	Sleep :: proc(milliseconds: c.int) ---
	Hash :: proc(hash: c.uint, data: ^u8, count: c.int) -> c.uint ---
	...
	World_Draw :: proc(worldId: WorldId, draw: ^DebugDraw, maskBits: c.ulong) ---
	...
	World_OverlapShape :: proc(worldId: WorldId, origin: Vec3, proxy: ^ShapeProxy, filter: QueryFilter, fcn: OverlapResultFcn, context_: rawptr) -> TreeStats ---
	...
	World_CastRay :: proc(worldId: WorldId, origin: Vec3, translation: Vec3, filter: QueryFilter, fcn: CastResultFcn, context_: rawptr) -> TreeStats ---
	...
	World_CastRayClosest :: proc(worldId: WorldId, origin: Vec3, translation: Vec3, filter: QueryFilter) -> RayResult ---
	...
	// @see b3World_CastRay
	World_CastShape :: proc(worldId: WorldId, origin: Vec3, proxy: ^ShapeProxy, translation: Vec3, filter: QueryFilter, fcn: CastResultFcn, context_: rawptr) -> TreeStats ---
	...
	World_CastMover :: proc(worldId: WorldId, origin: Vec3, mover: ^Capsule, translation: Vec3, filter: QueryFilter, fcn: MoverFilterFcn, context_: rawptr) -> f32 ---
	...
	World_CollideMover :: proc(worldId: WorldId, origin: Vec3, mover: ^Capsule, filter: QueryFilter, fcn: PlaneResultFcn, context_: rawptr) ---
	...
	RecPlayer_SetKeyframePolicy :: proc(player: ^RecPlayer, budgetBytes: c.ulong, minIntervalFrames: c.int) ---
	// @return the keyframe memory budget in bytes
	RecPlayer_GetKeyframeBudget :: proc(player: ^RecPlayer) -> c.ulong ---
	...
	RecPlayer_GetKeyframeBytes :: proc(player: ^RecPlayer) -> c.ulong ---
	...
	Body_GetPosition :: proc(bodyId: BodyId) -> Vec3 ---
	...
	Body_SetTransform :: proc(bodyId: BodyId, position: Vec3, rotation: Quat) ---
	// Get a local point on a body given a world point
	Body_GetLocalPoint :: proc(bodyId: BodyId, worldPoint: Vec3) -> Vec3 ---
	// Get a world point on a body given a local point
	Body_GetWorldPoint :: proc(bodyId: BodyId, localPoint: Vec3) -> Vec3 ---
	...
	Body_GetWorldPointVelocity :: proc(bodyId: BodyId, worldPoint: Vec3) -> Vec3 ---
	...
	Body_ApplyForce :: proc(bodyId: BodyId, force: Vec3, point: Vec3, wake: bool) ---
	...
	Body_ApplyLinearImpulse :: proc(bodyId: BodyId, impulse: Vec3, point: Vec3, wake: bool) ---
	...
	Body_GetWorldCenterOfMass :: proc(bodyId: BodyId) -> Vec3 ---
	...
	Body_CastRay :: proc(bodyId: BodyId, origin: Vec3, translation: Vec3, filter: QueryFilter, maxFraction: f32, bodyTransform: Transform) -> BodyCastResult ---
	// Cast a shape at a specific body using a specified body transform.
	Body_CastShape :: proc(bodyId: BodyId, origin: Vec3, proxy: ^ShapeProxy, translation: Vec3, filter: QueryFilter, maxFraction: f32, canEncroach: bool, bodyTransform: Transform) -> BodyCastResult ---
	// Overlap a shape with a specific body using a specified body transform.
	Body_OverlapShape :: proc(bodyId: BodyId, origin: Vec3, proxy: ^ShapeProxy, filter: QueryFilter, bodyTransform: Transform) -> bool ---
	// Collide a character mover with a specific body using a specified body transform.
	Body_CollideMover :: proc(bodyId: BodyId, bodyPlanes: ^BodyPlaneResult, planeCapacity: c.int, origin: Vec3, mover: ^Capsule, filter: QueryFilter, bodyTransform: Transform) -> c.int ---
	...
	Shape_RayCast :: proc(shapeId: ShapeId, origin: Vec3, translation: Vec3) -> CastOutput ---
